...
template <int TSLEEP> class Starter
{
    ...
    ~Starter() { join(long_mode); }

    void fork(int mode = 0)
    {
        if (!done) return;
        done = false;
        long_mode = mode;
    ...
    void join(int mode = 0)
    {
        if (done) return;
        if (long_mode != mode) return;
        done = true;
        trigger.start();
...
namespace OTFFT_MISC { typedef OktinyMP::Starter<0> okt; }
extern OTFFT_MISC::okt oktinymp;
...
static void init_Wq(const int N, complex_vector W) noexcept
{
    ...
    if (N == 1) {}
        ...
        const auto code = [&](int) noexcept {
            const int i0 = okt::i0(Ne);
            const int iN = okt::iN(Ne);
            for (int i = i0; i < iN; i++) {
                const int p = i + 1;
                const double theta = p * theta0;
                const double c =  cos(theta);
...
static void init_Wr(const int r, const int N, complex_vector W) noexcept
{
    ...
    if (N < OMP_THRESHOLD_W)
    {
        ...
        for (int k = 1; k < r; k++) {
            const auto code = [&](int) noexcept {
                const int i0 = okt::i0(Nr);
                const int iN = okt::iN(Nr);
                for (int p = i0; p < iN; p++) {
                    W[p + (k-1)*Nr] = expj(theta * k*p);
                }
            };
